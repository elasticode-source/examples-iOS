...
@property (nonatomic,assign) BOOL useRoundCorners __deprecated_msg("Use cornerRadius instead");
/** Button corner radius. @note Only available in CTAButtonType_custom */
...
+(NSArray*) updateServerAboutMoment:(NSString*) momentName elements:(NSArray*) elements group:(int) group momentType:(ECMomentType) momentType;
+(void) updateServerAboutMoment:(NSString*)momentName actions:(NSDictionary*) actionObjects elementsData:(NSArray*) elementsData momentType:(ECMomentType) momentType;
+(NSMutableArray*) getMoment:(NSString*) momentName elementsWithName:(NSString*) elementName frame:(CGRect) defaultFrame;
+(NSMutableArray*) getElementsWithName:(NSString*) elementName fromDictionary:(NSDictionary*) dictionary additionalAction:(NSArray*) additionalAction;

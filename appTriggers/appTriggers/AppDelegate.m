...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    // defining appTrigger with name, the when 'isActive' is true you can  show it with "showAppTrigger"
    // you should add the appTrigger from the dashboard first
    [params.data defineAppTrigger:@"appTrigger1" isActiveCallback:^(BOOL isActive) {
    
        if (isActive) {
            NSLog(@" now you can show the appTrigger");
        }
    }];
    [params.data defineAppTrigger:@"appTrigger2" isActiveCallback:^(BOOL isActive) {
        
        if (isActive) {
            NSLog(@" now you can show the appTrigger");
        }
    }];
    [params.data defineAppTrigger:@"appTrigger3" isActiveCallback:^(BOOL isActive) {
